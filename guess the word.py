...
# сколько она встречается в слове. Если такой буквы нет, к виселице добавляется круг в
# петле, изображающий голову. Пользователь продолжает отгадывать буквы до тех пор,
# пока не отгадает всё слово. За каждую неудачную попытку добавляется еще одна часть
# туловища висельника (обычно их 6: голова, туловище, 2 руки и 2 ноги).
...
def play(word):
    ...
    word=get_word()
    word_completion =[i for i in '_' * len(word)]  # строка, содержащая символы _ на каждую букву задуманного слова
    ...
    while 0<tries:
        s=input().lower()
        if not s.isalpha():
            print('Можно вводить только буквы.Попробуйте еще раз:')
            continue
        if s==word:
        ...
        for j in range(len(word)):
            if word[j]==s:
                word_completion[j]=s
        print('Смотрим, что получилось:', ' '.join(word_completion).upper())
...
def main():
    contin_game = input('Если хотите ещё сыграть, нажмите клавишу "д", не хотите нажмите "н": ')
    while True:
